...
def trainDoc2Vec(sentences):
    model = Doc2Vec(min_count=1, window=nGrams, size=doc2VecDims, sample=1e-4, negative=5, workers=8,iter=100,seed=1)
    model.build_vocab(sentences)
    model.train(sentences, total_examples=model.corpus_count, epochs=model.iter)
    return model
allText = allText.apply(keepTopWords,topWords=topWords)
sentences = constructLabeledSentences(allText)
doc2vecModel = trainDoc2Vec(sentences)
train_arrays = np.zeros((train.shape[0], doc2VecDims))
train_labels = np.zeros(train.shape[0])
for i in range(train.shape[0]):
    train_arrays[i] = doc2vecModel.docvecs['Text_'+str(i)]
    train_labels[i] = y[i]
...
doc2VecColNames =['DOC_'+(str(i)) for i in range(doc2VecDims)]
trainWithFeaturesForTraining = trainWithFeatures[colsForTraining]
docToVecDFTrain = pd.DataFrame(train_arrays)
docToVecDFTrain.columns = doc2VecColNames
trainWithFeaturesForTraining = pd.concat([trainWithFeaturesForTraining,docToVecDFTrain],axis=1,ignore_index=True)
trainWithFeaturesForTraining.columns = colsForTraining+doc2VecColNames
test_arrays = np.zeros((test.shape[0], doc2VecDims))
for i in range(train.shape[0],allText.shape[0]):
    test_arrays[i-train.shape[0]] = doc2vecModel.docvecs['Text_'+str(i)]
docToVecDFTest = pd.DataFrame(test_arrays)
docToVecDFTest.columns = doc2VecColNames
...
docToVecDFTest.reset_index(drop=True,inplace=True)
testWithFeaturesForPrediction = pd.concat([testWithFeaturesForPrediction,docToVecDFTest],axis=1,ignore_index=True)
testWithFeaturesForPrediction.columns = colsForTraining+doc2VecColNames
#train
denom = 0
...
submission_xgb.to_csv('submission_individual_tfidf'+str(doc2VecDims)+"_"+str(nGrams)+"_"+str(nTopWords)+".csv", index=False)
