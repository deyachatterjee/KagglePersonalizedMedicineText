...
train_text2 = pd.read_csv('data/test_text', sep="\|\|", engine='python', header=None, skiprows=1, names=["ID","Text"])
train_var1_class = pd.read_csv('data/stage1_solution_filtered.csv')
train_var1_class_np = train_var1_class.values
train_var1_class_ids = numpy.argmax((train_var1_class_np[:,1:]),axis=1)+1
train_var1_with_class_ids = train_var1[train_var1['ID'].isin(train_var1_class['ID'])]
train_text2_with_class_ids = train_text2[train_text2['ID'].isin(train_var1_class['ID'])]
train_var1_with_class_ids['Class'] =train_var1_class_ids
train_var = pd.concat([train_var,train_var1_with_class_ids]) 
...
model = Doc2Vec(min_count=1, window=5, size=embeddingDim, sample=1e-4, negative=5, workers=8,iter=100,seed=1)

model.build_vocab(sentences)

model.train(sentences, total_examples=model.corpus_count, epochs=model.iter)

model.save('./docEmbeddings_'+embeddingDim+'.d2v')

train_arrays = numpy.zeros((train.shape[0], embeddingDim))
